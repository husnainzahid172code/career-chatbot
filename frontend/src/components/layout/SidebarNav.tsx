...
export default function SidebarNav({ open, onToggle }: { open: boolean; onToggle: () => void }) {
  ...
  return (
    <>
      <aside className={`fixed inset-y-0 left-0 z-40 w-72 border-r border-subtle bg-page/90 p-4 backdrop-blur-2xl transition-transform duration-200 md:relative md:translate-x-0 ${
        open ? "translate-x-0" : "-translate-x-full"
      }`}>
        <div className="flex items-center justify-between">
          <div className="rounded-2xl bg-gradient-to-r from-indigo-500 to-fuchsia-500 p-[1px]">
            <div className="rounded-2xl bg-page p-3.5">
              <h1 className="text-lg font-bold text-primary">CareerPilot AI</h1>
              <p className="text-xs text-muted">Career & Internship Assistant</p>
            </div>
          </div>
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-all ${
                  isActive
                    ? "bg-gradient-to-r from-indigo-500/20 to-fuchsia-500/10 text-primary border border-indigo-500/20"
        ...
        <div className="absolute bottom-4 left-4 right-4 space-y-2">
          <button onClick={toggle} className="flex w-full items-center justify-between rounded-xl bg-card p-3 text-xs text-secondary hover:text-primary transition">
