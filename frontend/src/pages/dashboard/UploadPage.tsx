...
export default function UploadPage() {
  ...
  return (
    <div>
      <div className="mb-8">
        <h2 className="text-3xl font-bold bg-gradient-to-r from-emerald-400 to-cyan-400 bg-clip-text text-transparent">Resume & Document Analyzer</h2>
      ...
      <div className="rounded-2xl border border-subtle bg-card bg-gradient-to-br from-white/5 to-white/0 p-6 backdrop-blur-xl transition-colors duration-300">
        <div className="grid gap-6 md:grid-cols-2">
          <div>
            ...
            <div className="mt-2 flex items-center justify-center rounded-xl border-2 border-dashed border-subtle bg-input p-8 hover:border-indigo-500/50 transition cursor-pointer"
          ...
          <div>
            ...
            <textarea
              className="mt-2 h-32 w-full resize-none rounded-xl border border-subtle bg-input p-3 text-sm text-primary placeholder:text-disabled outline-none focus:border-emerald-500/50 transition"
        ...
        <div className="mt-5 flex gap-3">
          <button
            ...
            className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-emerald-500 to-cyan-500 px-6 py-2.5 text-sm font-semibold text-white hover:opacity-90 transition disabled:opacity-40"
          >
            {uploading ? (
          ...
          <button onClick={loadReports} className="flex items-center gap-2 rounded-xl border border-subtle px-5 py-2.5 text-sm text-secondary hover:bg-card hover:text-primary transition">
            <FiRefreshCw /> Refresh
          </button>
        </div>
      </div>
      ...
      {result && (
        <div className="mt-6 rounded-2xl border border-subtle bg-card bg-gradient-to-br from-white/5 to-white/0 p-6 backdrop-blur-xl transition-colors duration-300">
          <h3 className="mb-4 flex items-center gap-2 text-lg font-semibold">
            <FiCheckCircle className="text-emerald-400" /> Analysis Result
          </h3>
          <div className="grid gap-4 sm:grid-cols-3">
            <div className="rounded-xl bg-input p-4 text-center">
              ...
              <div className="mt-2 h-2 w-full overflow-hidden rounded-full bg-elevated">
                <div className={`h-full rounded-full transition-all duration-500 ${
                  result.atsScore >= 85 ? "bg-emerald-400" : result.atsScore >= 70 ? "bg-amber-400" : "bg-red-400"
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/10 p-4">
              <p className="flex items-center gap-1.5 text-sm font-medium text-emerald-400"><FiCheckCircle /> Strengths</p>
      ...
      <div className="mt-8">
        ...
        {reports.length === 0 ? (
          ...
          <div className="space-y-3">
            {reports.map((r: any) => (
              <div key={r._id} className="flex items-center justify-between rounded-xl border border-subtle bg-card p-4 hover:border-default transition">
                <div className="flex items-center gap-3">
                  <div className="rounded-lg bg-elevated p-2">
                    <FiUpload className="text-secondary" />
                  </div>
                  <div>
                    <p className="text-sm font-medium">{r.fileName || r.uploadFileId?.originalName || "Resume"}</p>
                    <p className="text-xs text-muted">{r.uploadedAt ? new Date(r.uploadedAt).toLocaleDateString() : ""}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <span className={`text-sm font-bold ${getScoreColor(r.atsScore)}`}>{r.atsScore}%</span>
                  <a
                    className="flex items-center gap-1 rounded-lg bg-indigo-500/20 px-3 py-1.5 text-xs text-indigo-400 hover:bg-indigo-500/30 transition"
